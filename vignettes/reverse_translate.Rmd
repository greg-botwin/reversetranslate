...
2. Allows user to provide their own Codon Frequency Table so codon selection can accurately mimic the desired translation system. Helper functions are provided so users can easily parse one of the 855,412 Codon Usage Tables species the  High-performance Integrated Virtual Environment-Codon Usage Tables (HIVE-CUTs) database provides [@Athey2017].
...
Users can easily create a Codon Frequency Table corresponding to their system of interest by searching the [HIVE-CUTs Database](https://hive.biochemistry.gwu.edu/dna.cgi?cmd=refseq_processor&id=569942).

For example, below we show the return search for Saccharomyces (4930). You will now want to copy and paste the complete Codon Usage Table including the two lines before the codon into a text file (see below for details.)

![HIVE-CUTs Search](example_hive_search_screen.png){width=600px}

Using the `build_hive_codon_tbl` function, users can easily read in a Codon Usage Table and convert it into a properly formatted Codon Frequency Table to be used for reverse translation. In addition to providing the path where the file is located, users are asked to provide the number of lines to skip before reading in the codon information (default = 2) and the total number of codons assessed in deriving the Codon Usage Table (in our test case = 5608057).

```{r, eval = FALSE}
sacchar_tbl <- build_hive_codon_tbl(file = "path/to/file.txt", skip = 2, 
                                    total_codons = 5608057)
```
...
In most cases you will not want to reverse translate a single amino acid sequence, but rather a peptide sequence. Lets take the below `minimal_aa_seq` as an example.
```{r}
minimal_aa_seq
```
...
```{r}
Biostrings::trinucleotideFrequency(Biostrings::DNAStringSet(codon_string), step = 3, simplify.as="collapsed")
```
